...
async function gotoGuide(anchor) {
    await gotoUrl(`${self.cfg.app.newcrewServerUrl}/guide#${anchor}`);
}

async function guideLogin() {
    await log('guideLogin');
    await gotoGuide('login');
}

async function guideValidate() {
    await log('guideValidate');
    await gotoGuide('validate');
}